...
import {
    Loader2, Crown, Clock, CheckCircle2, XCircle,
    User, RefreshCw, Filter, Search,
    Eye, Check, X, ImageIcon, CalendarDays, AlertCircle
} from "lucide-react"
...
const STATUS_STYLE = {
    ...
    COMPLETED: { label: "Diaktifkan", color: "text-white", bg: "bg-black", border: "border-black", icon: CheckCircle2 },
...
export default function AdminPremiumPage() {
    ...
    const fetchOrders = useCallback(async (q = "", p = 1, f = "", signal?: AbortSignal) => {
        ...
        try {
            ...
            const res = await fetch(`/api/premium?${params}`, { signal })
            if (!res.ok) {
                captureAPIError("/api/premium", res.status, res.statusText, { q, p, f })
                setFetchError(true)
                toast.error("Gagal memuat pesanan premium")
                return
            }
            ...
            if (latency > 1000) {
                capturePerformance("admin_premium_fetch_slow", latency, { q, p, f })
            }
        } catch (err: any) {
    ...
    // Combined stable AbortController search effect
    useEffect(() => {
    ...
    const handleAction = async (orderId: string, action: "COMPLETED" | "CANCELLED") => {
        captureInteraction("admin_premium_order_process_start", { orderId, action })
        ...
        toast.success(action === "COMPLETED" ? "Mengaktifkan premium..." : "Menolak pesanan...", { id: `premium-${orderId}` })
        ...
        try {
            ...
            const data = await res.json()
            if (!res.ok) {
                captureAPIError(`/api/premium/${orderId}`, res.status, res.statusText, { action })
                throw new Error(data.error || "Gagal memproses")
            }
            toast.success(action === "COMPLETED"
                ? `✅ Premium ${data.user?.name || "User"} telah diaktifkan!`
                : "❌ Order dibatalkan",
                { id: `premium-${orderId}` }
            )
            ...
            captureInteraction("admin_premium_order_process_success", { orderId, action })
        } catch (err: any) {
            setOrders(originalOrders)
            captureError(err, { context: "handleAction_rollback", orderId, action })
            captureInteraction("admin_rollback_event", { action: "admin_premium_order_process", targetId: orderId, error: err.message })
            toast.error(err.message || "Gagal memproses langganan. Dikembalikan ke awal.", { id: `premium-${orderId}` })
        } finally {
            setProcessing(null)
        }
    }
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <div className="w-6 h-6 rounded-md bg-fuchsia-300 border-[2px] border-black shadow-[2px_2px_0_#000] flex items-center justify-center">
                            <Crown className="w-3.5 h-3.5 text-black" />
                        </div>
                        <span className="text-[11px] font-black tracking-widest text-black uppercase">Admin Panel</span>
                    </div>
                    <h1 className="text-3xl lg:text-4xl font-black text-black tracking-tight uppercase">Pesanan Premium</h1>
                ...
                <button
                    ...
                    className="p-2.5 rounded-xl text-black bg-white border-[3px] border-black shadow-[4px_4px_0_#000] hover:bg-yellow-300 hover:translate-y-[1px] hover:shadow-[2px_2px_0_#000] transition-all self-start sm:self-auto"
                >
                    ...
                    return (
                        <button
                            ...
                            className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wide shrink-0 transition-all border-[2px] border-black ${
                                isActive
                                    ? "bg-fuchsia-300 text-black shadow-[3px_3px_0_#000] translate-x-[1px] translate-y-[1px]"
            ...
            {fetchError && (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-white border-[3px] border-black rounded-2xl shadow-[6px_6px_0_#000] space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-bold text-neutral-600 mt-1">Gagal mengambil data transaksi premium</p>
                    </div>
                    <button
                        onClick={() => fetchOrders(search, page, filter)}
                ...
                <AnimatePresence mode="popLayout">
                    ...
                    {orders.length === 0 && !loading && !fetchError && (
                        <div className="flex flex-col items-center justify-center py-20 text-center bg-white border-[3px] border-black rounded-2xl shadow-[8px_8px_0_#000]">
                            <div className="w-16 h-16 bg-fuchsia-300 border-[3px] border-black rounded-2xl shadow-[4px_4px_0_#000] flex items-center justify-center mb-4 -rotate-3">
                                <Crown className="w-8 h-8 text-black" />
                            </div>
                            <p className="text-xl font-black text-black uppercase tracking-wide">Belum ada pesanan premium</p>
                        </div>
                    )}
                    ...
                    {!loading && orders.map((order, i) => {
                        ...
                        return (
                            ...
                            >
                                {/* Avatar */}
                                <div className="w-12 h-12 rounded-xl bg-violet-100 overflow-hidden border-[2px] border-black shadow-[2px_2px_0_#000] shrink-0 flex items-center justify-center relative">
                                    {order.user.image ? (
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                        ...
                                        {order.user.isPremium && (
                                            <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border-[2px] border-black bg-fuchsia-300 text-black shadow-[2px_2px_0_#000]">
                                                ✨ Premium
                                            </span>
                                        )}
                                        <span
                                    ...
                                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                        <span className="text-[11px] bg-fuchsia-300 text-black px-2 py-0.5 rounded-lg border-[2px] border-black font-black uppercase flex items-center gap-1 shadow-[2px_2px_0_#000]">
                                            <CalendarDays className="w-3 h-3" />
                                            {order.durationDays} hari
                                        </span>
                                        ...
                                        {order.proofImage && (
                                            <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg flex items-center gap-1 bg-yellow-300 text-black border-[2px] border-black shadow-[2px_2px_0_#000]">
                                ...
                                {order.status === "PENDING" && (
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        >
                                            {isOrderProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 inline mr-1 font-black" />Aktifkan</>}
                                        </button>
                                        <button
            ...
            <AnimatePresence>
                {selectedOrder && (
                    ...
                    >
                        ...
                        >
                            <div className="px-5 py-4 border-b-[3px] border-black bg-fuchsia-300 flex justify-between items-center">
                                <div>
                                    <h2 className="font-black text-black text-lg uppercase">Detail Pesanan Premium</h2>
                            ...
                            <div className="p-5 space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-wider mt-1 px-2 py-0.5 rounded-lg border-[2px] border-black inline-block shadow-[2px_2px_0_#000]" style={{ backgroundColor: selectedOrder.user.isPremium ? "#f0abfc" : "#f3f4f6", color: "#000" }}>
                                            {selectedOrder.user.isPremium
                                                ? `✨ Aktif s.d. ${selectedOrder.user.premiumExpiresAt ? new Date(selectedOrder.user.premiumExpiresAt).toLocaleDateString("id-ID") : "-"}`
                                                : "Belum premium"
                                            }
                                        </p>
                                    </div>
                                ...
                                <div className="rounded-xl p-4 bg-white border-[3px] border-black shadow-[4px_4px_0_#000]">
                                    <div className="flex justify-between mb-2 pb-2 border-b-[2px] border-dashed border-neutral-300">
                                        <span className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Durasi</span>
                                        <span className="text-sm font-black text-black bg-fuchsia-300 px-2 py-0.5 rounded-lg border-[2px] border-black shadow-[2px_2px_0_#000]">{selectedOrder.durationDays} hari</span>
                                    </div>
                                    <div className="flex justify-between mb-2 pb-2 border-b-[2px] border-dashed border-neutral-300">
                                        <span className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Harga</span>
                                        <span className="text-sm font-black text-black">{formatRupiah(selectedOrder.price)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Bonus</span>
                                        <span className="text-sm font-black text-black bg-yellow-300 px-2 py-0.5 rounded-lg border-[2px] border-black shadow-[2px_2px_0_#000]">
                                            {selectedOrder.user.isPremium ? "—" : "+250 poin"}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {selectedOrder.proofImage ? (
                                    ...
                                    >
                                        ...
                                        <p className="text-sm font-bold text-neutral-500">Bukti pembayaran belum diunggah</p>
                                    </div>
                                )}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Misal: Pembayaran sudah dikonfirmasi"
                                ...
                                <div className="flex gap-3 pt-2">
                                    ...
                                    >
                                        {processing ? <Loader2 className="w-4 h-4 animate-spin inline" /> : <><Crown className="w-4 h-4 inline mr-1 font-black" />Aktifkan Premium</>}
                                    </button>
                                </div>
                            </div>
                        </motion.div>
