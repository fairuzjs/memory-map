...
import {
    ...
    Eye, Check, X, ImageIcon, CalendarDays
} from "lucide-react"
...
const STATUS_STYLE = {
    ...
    COMPLETED: { label: "Diaktifkan", color: "text-emerald-400", bg: "rgba(52,211,153,0.08)", border: "rgba(52,211,153,0.2)", icon: CheckCircle2 },
...
export default function AdminPremiumPage() {
    ...
    const fetchOrders = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/premium?${params}`)
    ...
    const handleAction = async (orderId: string, action: "COMPLETED" | "CANCELLED") => {
        ...
        try {
            ...
            if (action === "COMPLETED") {
                toast.success(data.message || "Premium diaktifkan!")
            } else {
        ...
        <div className="space-y-6" style={{ fontFamily: "Outfit, sans-serif" }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-white">Pesanan Premium</h1>
                ...
                <button
                    onClick={fetchOrders}
                    className="p-2 rounded-xl text-neutral-500 hover:text-white hover:bg-white/[0.06] transition-colors self-start"
                >
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
                ...
                {STATUS_FILTER_OPTS.map(opt => (
                    <button
                        ...
                        style={{
                            background: filter === opt.value ? "rgba(139,92,246,0.15)" : "rgba(255,255,255,0.04)",
                            border: filter === opt.value ? "1px solid rgba(139,92,246,0.4)" : "1px solid rgba(255,255,255,0.07)",
                            color: filter === opt.value ? "#a78bfa" : "#6b7280",
                        }}
                    >
                        {opt.label}
                    </button>
                ...
                <AnimatePresence mode="popLayout">
                    {orders.length === 0 && !loading ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <Crown className="w-10 h-10 text-neutral-700 mb-3" />
                            <p className="text-neutral-500 text-sm">Belum ada pesanan premium</p>
                        </div>
                    ) : (
                        orders.map((order, i) => {
                                ...
                                >
                                    ...
                                    {order.status === "PENDING" && (
                                        <div className="flex items-center gap-2 shrink-0">
                                            ...
                                            >
                                                {processing === order.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <><Check className="w-3.5 h-3.5 inline mr-1" />Aktifkan</>}
                                            </button>
                                            <button
                    ...
                    >
                        ...
                        >
                            <div className="p-5 border-b" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
                                <h2 className="font-bold text-white">Detail Pesanan Premium</h2>
                            ...
                            <div className="p-5 space-y-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs mt-0.5" style={{ color: selectedOrder.user.isPremium ? "#a78bfa" : "#6b7280" }}>
                                            {selectedOrder.user.isPremium
                                                ? `✨ Premium aktif (s.d. ${selectedOrder.user.premiumExpiresAt ? new Date(selectedOrder.user.premiumExpiresAt).toLocaleDateString("id-ID") : "-"})`
                                                : "Belum premium"
                                            }
                                        </p>
                                    </div>
                                ...
                                <div className="rounded-xl p-3" style={{ background: "rgba(139,92,246,0.05)", border: "1px solid rgba(139,92,246,0.12)" }}>
                                    <div className="flex justify-between mb-1">
                                        ...
                                        <span className="text-xs font-bold text-violet-400">{selectedOrder.durationDays} hari</span>
                                    </div>
                                    <div className="flex justify-between mb-1">
                                    ...
                                    <div className="flex justify-between">
                                        <span className="text-xs text-neutral-500">Bonus upgrade</span>
                                ...
                                {selectedOrder.proofImage ? (
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-neutral-600">Bukti pembayaran belum diunggah oleh user</p>
                                    </div>
                                )}
                                ...
                                <div className="flex gap-2 pt-1">
                                    ...
                                    >
                                        {processing ? <Loader2 className="w-3.5 h-3.5 animate-spin inline" /> : <><Crown className="w-3.5 h-3.5 inline mr-1" />Aktifkan Premium</>}
                                    </button>
                                </div>
                            </div>
                        </motion.div>
                ...
                {proofModalUrl && (
                    ...
                    >
                        ...
                        >
                            ...
                            <p className="text-xs text-neutral-500 text-center mb-3">Bukti Pembayaran</p>
                            <img
                                src={proofModalUrl}
                                alt="Bukti pembayaran"
